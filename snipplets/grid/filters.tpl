...
{% else %}
    {% if product_filters is not empty %}
        ...
        <div id="filters" class="visible-when-content-ready" data-store="filters-nav">
            {% if not modal %}
                <h5 class="font-large mb-2 d-none d-md-block">{{ "Filtrar por" | translate }}</h5>
            {% endif %}
            {% for product_filter in product_filters %}
                {% if product_filter.type == 'price' %}
                    ...
                    {{ component(
                        'price-filter',
                        {'group_class': 'price-filter-container', 'title_class': 'mb-3 mt-4 subtitle', 'button_class': 'btn btn-secondary btn-small ' ~  btn_price_classes }
                    ) }}
                                    ...
                                    <label class="js-filter-checkbox {% if not value.selected %}js-apply-filter{% else %}js-remove-filter{% endif %} checkbox-container" data-filter-name="{{ product_filter.key }}" data-filter-value="{{ value.name }}" data-component="filter.option" data-component-value="{{ value.name }}">
                                        ...
                                        <span class="checkbox">
                                            ...
                                            <span class="checkbox-text with-color">
                                                {{ value.name }} <span class="opacity-50 font-small">({{ value.product_count }})</span>
                                            </span>
