{% set inline_quick_shop = settings.quick_shop_type == 'inline' %}
{% set quick_shop_variants_class = inline_quick_shop ? 'quickshop-variants' : 'mb-3' %}
{% set product_variant_group_class = inline_quick_shop ? 'mb-2' %}

<div class="js-product-variants {% if quickshop %}js-product-quickshop-variants {{ quick_shop_variants_class }}{% endif %} form-row{% if not (quickshop and inline_quick_shop) %} pt-2{% endif %}">
	...
	{% for variation in product.variations %}
		...
		<div class="js-product-variants-group {% if variation.name in ['Color', 'Cor'] %}js-color-variants-container{% endif %} {% if quickshop %}col-12 {{ product_variant_group_class }} {% else %} col-12 {% if settings.bullet_variants %}mb-2{% else %}{% if loop.length == 3 %} col-md-4 {% else %} col-md-6{% endif %}{% endif %}{% endif %}" data-variation-id="{{ variation.id }}">
			{% if quickshop %}
				{% embed "snipplets/forms/form-select.tpl" with{select_label: true, select_label_name: '' ~ variation.name ~ '', select_for: 'variation_' ~ loop.index , select_id: 'variation_' ~ loop.index, select_name: 'variation' ~ '[' ~ variation.id ~ ']', select_group_custom_class: 'mb-2' ~ hidden_variant_select, select_custom_class: 'js-variation-option js-refresh-installment-data form-control-small', select_label_custom_class:'mb-1'} %}
