...
{% elseif template == 'product' %}
    ...
    {% for media in product.media %}
        {% if media.isImage and loop.first %}
            <link rel="preload" as="image" href="{{ media | product_image_url('large') }}" imagesrcset="{{ media | product_image_url('large') }} 480w, {{ media | product_image_url('huge') }} 640w, {{ media | product_image_url('original') }} 1024w">
        {% endif %}
    {% endfor %}
